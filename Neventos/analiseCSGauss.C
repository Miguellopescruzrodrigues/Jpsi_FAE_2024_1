...
void plotDifferentialCrossSection(const vector<double>& pt_bins, const vector<double>& diff_cross_sections, const vector<double>& errors) {
    ...
    TCanvas *c1 = new TCanvas("c_diff_cross_section", "Differential Cross Section", 800, 600);
...
void analiseCSGauss() {
    ...
    hJpsiMassCut->GetYaxis()->SetTitle("Events");
        ...
        TH1S *hDiMuPT = new TH1S("DiMuPT", "", 200, 0., 60.); 
    hDiMuPT->GetXaxis()->SetTitle("DIMuonP_{T} (GeV)");
    hDiMuPT->GetYaxis()->SetTitle("Events");


        TH1S *hDiMuPTCutMass = new TH1S("hDiMuPTCutMass", "", 200, 0., 60.); 
    hDiMuPTCutMass->GetXaxis()->SetTitle("DiMuon p_{T} (GeV)");
    hDiMuPTCutMass->GetYaxis()->SetTitle("Events");
    ...
    vector<double> yields(10, 0.0);
    vector<double> yieldErrors(10, 0.0);
    vector<double> pt_bins = {1., 3., 5., 7., 9., 11., 13., 15., 28., 50., 60.};
 
    // Contadores para diferentes faixas de pt
    vector<int> pt_counts(11, 0);
    vector<string> pt_labels = {"1-3", "3-5", "5-7", "7-9", "9-11", "11-13", "13-15", "15-30", "30-40", "40-50", "50+"};
    ...
    for(int i = 0; i < nentries; i++) {
        ...
        double pt1Cut = muonN_p4->Pt();
        double pt2Cut = muonP_p4->Pt();
        double DiMuPt = dimuon_p4->Pt();
        ...
        if(pt1 > 1 && pt2 > 1 && etacut < 2.4) {
                hDiMuPT->Fill(pt1);
                hDiMuPT->Fill(pt2);
            if(mass >= 2.9 && mass <= 3.3){
                n_pass_cuts++;
                hJpsiMassCut->Fill(mass); // Preencher o histograma principal
                hDiMuPTCutMass->Fill(DiMuPt);
                //hDiMuPTCutMass->Fill(pt2Cut);
                // Preencher o histograma correto para o intervalo de pt
                if (pt3 >= 1 && pt3 < 3) {
                    hists[0]->Fill(mass);
                    pt_counts[0]++;
                    ...
                    hists[3]->Fill(mass);
                    pt_counts[3]++;
                } else if (pt3 >= 9 && pt3 < 11) {
                    hists[4]->Fill(mass);
                    pt_counts[4]++;
                } else if (pt3 >= 11 && pt3 < 13) {
                    hists[5]->Fill(mass);
                    pt_counts[5]++;
                } else if (pt3 >= 13 && pt3 < 15) {
                    hists[6]->Fill(mass);
                    pt_counts[6]++;
                } else if (pt3 >= 15 && pt3 < 30) {
                    hists[7]->Fill(mass);
                    pt_counts[7]++;
                } else if (pt3 >= 30 && pt3 < 40) {
                    hists[8]->Fill(mass);
                    pt_counts[8]++;
                } else if (pt3 >= 40 && pt3 < 50) {
                    hists[9]->Fill(mass);
                    pt_counts[9]++;
                } else if (pt3 >= 50) {
                    hists[10]->Fill(mass);
    ...
    TCanvas *PtCanvas = new TCanvas("PtCanvas","PtCanvas",5,5,1000,1000);
    PtCanvas->SetLeftMargin(0.15); // Aumenta a margem esquerda para evitar o corte do eixo Y
    hDiMuPT->Draw();
    hDiMuPT->SetMarkerStyle(12);
    hDiMuPT->SetMarkerColor(10);

        TLatex latexPt;
    latexPt.SetNDC(); // Usar coordenadas normalizadas
    latexPt.SetTextSize(0.03); // Tamanho do texto
    latexPt.SetTextFont(42); // Fonte normal (sem negrito)
    latexPt.DrawLatex(0.15, 0.9, "#bf{CMS} #it{Open Data}");
    latexPt.SetTextSize(0.03); // Tamanho do texto
    latexPt.SetTextFont(42); // Fonte normal (sem negrito)
    latexPt.DrawLatex(0.20, 0.75, "Reconstructed mass cut:");


    TCanvas *PtCutCanvas = new TCanvas("PtCutCanvas","PtCutCanvas",5,5,1000,1000);
    PtCutCanvas->SetLeftMargin(0.15); // Aumenta a margem esquerda para evitar o corte do eixo Y
    hDiMuPTCutMass->Draw();
    hDiMuPTCutMass->SetMarkerStyle(12);
    hDiMuPTCutMass->SetMarkerColor(10);

        TLatex latexPtcut;
    latexPtcut.SetNDC(); // Usar coordenadas normalizadas
    latexPtcut.SetTextSize(0.03); // Tamanho do texto
    latexPtcut.SetTextFont(42); // Fonte normal (sem negrito)
    latexPtcut.DrawLatex(0.15, 0.9, "#bf{CMS} #it{Open Data}");
    latexPtcut.SetTextSize(0.03); // Tamanho do texto
    latexPtcut.SetTextFont(42); // Fonte normal (sem negrito)
    latexPtcut.DrawLatex(0.20, 0.75, "Reconstructed mass cut:");
    latexPtcut.DrawLatex(0.25, 0.72, "#bf{2.9<mass<3.3} ");
